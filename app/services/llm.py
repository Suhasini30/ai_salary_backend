...
class LLMService:
    ...
    def chat(self, prompt, model=None):
        ...
        for candidate in [settings.XAI_MODEL, settings.GROQ_MODEL, settings.DEFAULT_MODEL]:
            if candidate:
        ...
        for idx, current_raw in enumerate(fallback_models):
            ...
            if api_key is None and any(p in current_model.lower() for p in ["xai", "grok", "groq", "gemini"]):
